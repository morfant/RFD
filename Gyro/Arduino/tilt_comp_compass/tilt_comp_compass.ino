...
int     Mag_x_offset = 8191,      Mag_y_offset = 7031,     Mag_z_offset = -4664;   // Hard-iron offsets
float   Mag_x_scale = 1.01,     Mag_y_scale = 1.26,     Mag_z_scale = 0.82;    // Soft-iron scale factors
float   ASAX = 1.17,            ASAY = 1.18,            ASAZ = 1.13;           // (A)sahi (S)ensitivity (A)djustment fuse ROM values.
...
//   Serial.print("ap: ");;
//   Serial.println(Gyro_pitch_output * 10);
//   Serial.print("ar: ");
//   Serial.println(Gyro_roll_output * 10);
  Accel_pitch -= 0.0f;                                             //Accelerometer calibration value for pitch
  Accel_roll = 0.0f;                                               //Accelerometer calibration value for roll


  // Accel_pitch += 14.3f;                                             //Accelerometer calibration value for pitch
  // Accel_roll += 8.6f;                                               //Accelerometer calibration value for roll
  ...
  */
  ...
  Mag_x_dampened = Mag_x_dampened * 0.5 + Mag_x_hor * 0.5;
  Mag_y_dampened = Mag_y_dampened * 0.5 + Mag_y_hor * 0.5;
  ...
  Serial.print(Mag_z);
  Serial.print(" ");
  Serial.print(Mag_pitch * RAD_TO_DEG);
  Serial.print(" ");
  Serial.print(Mag_roll * RAD_TO_DEG);
  Serial.print(" ");
