...
#define MAG_CALIBRATION true
#define CaliPlot false
...
void setup()
{
  ...
  if (c == 0x71) // WHO_AM_I should always be 0x68
  {
    ...
    Serial.println("magCalibration:"); Serial.println(magCalibration[0]); Serial.println(magCalibration[1]); Serial.println(magCalibration[2]);

    if (MAG_CALIBRATION) {
      getMres();
      magcalMPU9250(magBias, magScale);

      Serial.println("magBias:"); Serial.println(magBias[0]); Serial.println(magBias[1]); Serial.println(magBias[2]);
      Serial.println("magScale:"); Serial.println(magScale[0]); Serial.println(magScale[1]); Serial.println(magScale[2]);
...
void loop()
{
  ...
  if (!AHRS) {
    ...
    delt_t = millis() - count;
    if (delt_t > interval) { // update LCD once per half-second independent of read rate
      ...
      yaw   += 8.99; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
      roll  *= 180.0f / PI;
...
void initAK8963(float * destination)
{
  ...
  Serial.println("rawData: "); Serial.println(rawData[0]); Serial.println(rawData[1]); Serial.println(rawData[2]); 
...
void magcalMPU9250(float * dest1, float * dest2)
{
  ...
  Serial.println("mag_bias:"); Serial.println(mag_bias[0]); Serial.println(mag_bias[0]); Serial.println(mag_bias[2]);
  Serial.println("mRes:"); Serial.println(mRes);
  Serial.println("magCalibration:"); Serial.println(magCalibration[0]); Serial.println(magCalibration[1]); Serial.println(magCalibration[2]);
